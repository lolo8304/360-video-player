...
- (void)incFingerRotationX: (CGFloat) x Y: (CGFloat) y {
    [self setFingerRotationX: (self.fingerRotationX + x) Y: (self.fingerRotationY + y)];
//    NSLog(@"change finger rotation X=%.6g, Y=%.6g", x, y);
...
//  if(self.isUsingMotion) return;
    ...
    incFingerRotX = 0;
    incFingerRotY = 0.001417 * 5;
    */
    
    [self incFingerRotationX: incFingerRotX Y: incFingerRotY];
}
